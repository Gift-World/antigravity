...
export const AlertPanel: React.FC = () => {
  const { alerts, acknowledgeAlert, dismissAlert, triggerAlert, activeEventId } = useAppStore();
  ...
  const handleQuickAction = (alert: Alert, actionType: string) => {
    ...
    if (actionType === 'open_gate') {
      triggerAlert({
        ...
        message: `DIRECTIVE DISPATCHED: Open emergency relief gates immediately.`,
        severity: 'warning',
      ...
      triggerAlert({
        ...
        alert_type: 'custom',
        zone_id: alert.zone_id,
        message: `PARAMEDIC DISPATCHED: Medical post responding with mobile trauma kit.`,
        severity: 'warning',
      ...
      <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
        {alerts.length === 0 ? (
          <div className="h-40 flex flex-col items-center justify-center text-ag-text-muted text-xs">
            ...
            <span>All zones nominal. No active alerts.</span>
          </div>
        ) : (
          alerts.map((alert) => {
            ...
            return (
              <div
                ...
                className={`p-3 rounded-[6px] border transition-all duration-200 ${
                  isCritical
                    ? 'bg-ag-red-dim border-ag-red/40 shadow-sm'
                    : isWarning
                    ? 'bg-ag-yellow-dim border-ag-yellow/40'
                    : 'bg-ag-black/40 border-ag-border'
                ...
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    {alert.auto_generated && (
                      <span className="text-[9px] font-mono text-ag-text-muted border border-ag-border px-1 py-0.2 rounded">
                        AUTO-AI
                      </span>
                    )}
                  </div>
