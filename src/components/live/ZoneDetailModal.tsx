...
import { X, TrendingUp, Shield, DoorOpen } from 'lucide-react';
...
export const ZoneDetailModal: React.FC<ZoneDetailModalProps> = ({ zone, reading, onClose }) => {
  ...
  return (
    <div className="absolute top-4 right-4 w-96 max-w-[calc(100vw-2rem)] bg-ag-surface/95 border border-ag-border backdrop-blur-xl rounded-2xl shadow-2xl p-5 z-40 animate-in fade-in slide-in-from-right-4 duration-200 font-sans">
      {/* Header */}
      <div className="flex items-start justify-between border-b border-ag-border pb-3 mb-3">
        <div>
          <div className="flex items-center gap-2">
            <h4 className="font-display font-bold text-base text-white">{zone.name}</h4>
            <span
              className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${color.badgeBg} ${color.badgeBorder} ${color.badgeText}`}
            >
              {percentCapacity >= 90 ? 'Danger' : percentCapacity >= 70 ? 'Caution' : 'Safe'}
            </span>
          </div>
          <p className="text-xs text-ag-text-secondary mt-0.5">
            Capacity: {formatNumber(zone.capacity)} max
          </p>
        </div>
        <button
          onClick={onClose}
          className="text-ag-text-muted hover:text-white p-1 rounded-lg hover:bg-ag-surface-hover"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="bg-ag-black/50 p-3 rounded-xl border border-ag-border text-center">
          <div className="text-[11px] text-ag-text-muted font-semibold">People Inside</div>
          <div className="font-display font-bold text-xl text-white mt-0.5">
            {formatNumber(reading.estimated_count)}
          </div>
        </div>

        <div className="bg-ag-black/50 p-3 rounded-xl border border-ag-border text-center">
          <div className="text-[11px] text-ag-text-muted font-semibold">Capacity Full</div>
          <div className="font-display font-bold text-xl text-ag-green mt-0.5">
            {percentCapacity}%
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 bg-ag-black/40 p-3 rounded-xl border border-ag-border">
        <div className="flex items-center justify-between text-xs text-ag-text-secondary mb-2">
          <span className="flex items-center gap-1 font-semibold text-white">
            <TrendingUp className="w-3.5 h-3.5 text-ag-green" /> 30-Min Flow Trend
          </span>
        </div>
      ...
      <div className="space-y-2">
        <div className="text-xs font-semibold text-ag-text-muted">
          Quick Actions
        </div>
